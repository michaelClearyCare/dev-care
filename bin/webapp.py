...
class Model:
  ...
  def load(self):
    self.config = json.loads(file('etc/actions.json').read())
...
class HttpHandler( BaseHTTPServer.BaseHTTPRequestHandler ):
  ...
  def do_POST(self):
    ...
    # In the child process, set up GO_* environmental vars for POST data.
    pid = os.fork()
...
def main( argv ):
  ...
  print >> sys.stderr, "Go server @ http://%s:%u/" % (host,port)
